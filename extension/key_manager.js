        ...
        */
        ...
        for (var inputElem in inputsToLabel) {
            ...
            if (input.id !== undefined) {
                input.nextSibling.textContent = (input.name === "sign") ? _("Sign") :
                    (input.name === "enc") ? _("Encrypt") :
                    (input.name === "auth") ? _("Authenticate") : "";
            }
        }
                                ...
                                'html': function() {
                                    ...
                                    for (var uid in sres[xkey].uids) {
                                        ...
                                        if (sres[xkey].uids[uid].uid.length > 1)
                                            uidlist += webpg.utils.escape(sres[xkey].uids[uid].uid) + " - ";
                                        if (sres[xkey].uids[uid].email.length > 1)
                                            uidlist += webpg.utils.escape(sres[xkey].uids[uid].email);
                                        uidlist += "</li>";
                                    }
                                    uidlist += "</ul>";
                                    return uidlist;
    ...
    */
    ...
    progressMsg: function(evt) {
        ...
        if (dialog && msg.type === "progress") {
            ...
            data += " ";
            webpg.jq(dialog + "_progress").append(webpg.utils.escape(data));
                    ...
                    webpg.jq("#gensubkey-dialog").dialog({
                        ...
                        'buttons': [
                        {
                            ...
                            'click': function() {
                                ...
                                webpg.keymanager.genkey_waiting = true;
                                if (webpg.utils.detectedBrowser.product === "chrome") {
                                    chrome.extension.onConnect.addListener("gpgGenKeyProgress", function(port) {
                                        port.onMessage.addListener("gpgGenKeyProgress", webpg.keymanager.progressMsg);
                                    });
                                }
